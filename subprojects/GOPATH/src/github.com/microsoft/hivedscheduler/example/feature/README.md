# Feature Demo
1. [OpenPAI](https://github.com/microsoft/pai) is also leveraged to show these end to end scheduling feature demos, however, all these features can also be reproduced by other platforms or even by [raw pods](../request/basic/request.yaml).
2. HiveD config and Job request may need to adjust according to your own cluster.

## VC Safety
### Description
VC's quota can be described by Hardware Quantity, [Topology](#VC-Safety), [Type](#GPU-Type), [Reservation](#Reservation), etc. To guarantee all VCs' quota, for example, one VC's [guaranteed usage](#Guaranteed-Job) should never make fragmentation inside other VCs' quota:

Two DGX-2s, two VCs each owns one DGX-2 node. For normal scheduler, this will translate into two VCs each owns 16 GPUs. When user submits 16 1-GPU jobs to VC1, the user in VC2 might not be able to run a 16-GPU job, due to possible fragmentation issue caused by VC1. While HiveD can always guarantee each VC has one entire node reserved for its dedicated use.

### Reproduce Steps
1. Using [hived-config-1](file/hived-config-1.yaml).
2. Submit 2 jobs [itc-safety-1](file/itc-safety-1.yaml), [itc-safety-2](file/itc-safety-2.yaml) to the same VC, all tasks will also always within one node (10.151.41.26).
...
### Reproduce Steps
#### `gpuType` specified
1. Using [hived-config-2](file/hived-config-2.yaml).
2. Submit job [itc-k80-type](file/itc-k80-type.yaml), it will still partially run.
   <img src="file/itc-k80-type.png" width="900"/>

#### `gpuType` not specified
1. Using [hived-config-2](file/hived-config-2.yaml).
2. Submit job [itc-no-type](file/itc-no-type.yaml), it will be fully running, and one node will be 10.151.41.26 (M60 type).
...
This is useful for job which cannot perform any useful work, such as making progress or serving, until all pods are running.

### Reproduce Steps
#### Basic
1. Using [hived-config-2](file/hived-config-2.yaml).
2. Submit job [itc-gang](file/itc-gang.yaml), which requests for 6 single GPU tasks, all tasks will be waiting without IP associated.
3. Submit job [itc-gang4](file/itc-gang4.yaml), which requests for 4 single GPU tasks, all tasks will be running, while [itc-gang](file/itc-gang.yaml) will be still waiting without IP associated. This also shows that there is no [head-of-line blocking](https://en.wikipedia.org/wiki/Head-of-line_blocking) by HiveD itself.
...
A job can specify all its pods are in different `AffinityGroups`, so the whole job is incremental scheduled.

This is used for job which can still perform useful works, such as making progress or serving, even if only one pod is running.
...
## Opportunistic Job
### Description
Opportunistic Job: Job whose priority is -1, it can use other VCs' quota, however, once it is allocated, it may be preempted by other VCs' jobs.

### Reproduce Steps
1. Using [hived-config-1](file/hived-config-1.yaml).
2. Submit job [itc-oppo](file/itc-oppo.yaml), it will use more than one node, even if its VC1's quota cannot hold it.
   <img src="file/itc-oppo.png" width="900"/>

## Intra VC Preemption
### Description
Within one VC, high priority job can preempt low priority jobs.

### Reproduce Steps
#### Immediate Preemption
1. Using [hived-config-3](file/hived-config-3.yaml).
...
### Description
Within one VC, best effort to choose near GPUs for one job.
...
4. Using [hived-config-33](file/hived-config-33.yaml), and restart HiveD. 
...
1. Using [hived-config-33](file/hived-config-33.yaml).
...
4. Using [hived-config-2](file/hived-config-2.yaml), and restart HiveD. 
...
4. Using [hived-config-4](file/hived-config-4.yaml), and restart HiveD. 
...
4. Using [hived-config-7](file/hived-config-7.yaml), and restart HiveD. 
...
4. Using [hived-config-5](file/hived-config-5.yaml), and restart HiveD. 
...
4. Using [hived-config-6](file/hived-config-6.yaml), and restart HiveD. 
5. The job will still run without any interruption but [lazy preempted](#Lazy-Preemption) by HiveD.
...
### Reproduce Steps
1. Using [hived-config-2](file/hived-config-2.yaml).
2. Stop kubelet on 10.151.41.26 (the only M60 node) by `sudo systemctl stop kubelet`. Waiting until this is detected by K8S.
...
4. Bring back 10.151.41.26 by `sudo systemctl start kubelet`. Waiting until this is detected by K8S.
