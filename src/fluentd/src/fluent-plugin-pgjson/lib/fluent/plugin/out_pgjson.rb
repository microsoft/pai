      ...
      if ! thread[:conn].nil?
        begin
          chunk.msgpack_each do |time, record|
            ...
            if kind == "Framework"
              thread[:conn].exec("COPY framework_history (\"#{@insertedAt_col}\", \"#{@uid_col}\", \"#{@frameworkName_col}\", \"#{@attemptIndex_col}\", \"#{@historyType_col}\", \"#{@snapshot_col}\") FROM STDIN WITH DELIMITER E'\\x01'")
              ...
              thread[:conn].put_copy_data "#{time}\x01#{uid}\x01#{frameworkName}\x01#{attemptIndex}\x01#{historyType}\x01#{snapshot}\n"
            elsif kind == "Pod"
