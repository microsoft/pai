...
function reducer(state, action) {
  ...
  switch (action.type) {
    case 'dataList':
      jobData = new JobData(state.hdfsClient, action.value, state.mountDirs, true);
      action.onChange(jobData);
      return jobData;
    case 'mountDir':
      jobData = new JobData(state.hdfsClient, state.customDataList, action.value, true);
      action.onChange(jobData);
      return jobData;
    default:
...
export const DataComponent = React.memo((props) => {
  ...
  const [jobData, dispatch] = useReducer(reducer, new JobData(hdfsClient, [], null));

  const _onDataListChange = useCallback((dataList) => {
    dispatch({type: 'dataList', value: dataList, onChange: onChange});
  }, [onChange]);

  const onMountDirChange = useCallback((mountDir) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <CustomStorage dataList={jobData.customDataList} setDataList={_onDataListChange} />
        <MountTreeView
          dataList={
            jobData.mountDirs == null
              ? jobData.customDataList
              : jobData.mountDirs.getTeamDataList().concat(jobData.customDataList)
          }
        />
      </SidebarCard>
    </HdfsContext.Provider>
