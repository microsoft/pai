...
class JobManager(object):
    def __init__(self, username=settings.PAI_username, password=settings.PAI_password, expiration=30000):
        ...
        self.rest_server_socket = settings.rest_server_socket  # rest server socket
        self.http = urllib3.PoolManager()  # urllib3 http
        self.token = get_token(username, password, expiration)  # rest Server token
        self.hdfs_client = Client(settings.hdfs_socket)  # hdfs web socket
    ...
    def get_status(self, job_name="", has_service=False, service='main'):
        ...
        if return_json["state"] == "FAILED":
            ...
            if return_json["exit_type"] == "NON_TRANSIENT" or return_json["exit_type"] == "UNKNOWN":
                return_json["log"] = []
                for task in data["taskRoles"]["taskStatus"]:
