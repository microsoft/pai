...
训练不同参数的模型是很常见的。OpenPAI支持参数定义和参考，这提供了一种灵活的方式来训练和比较模型。你可以在 `Parameters` 部分定义参数，并在命令中使用 `<% $parameters.paramKey %>` 来引用它们。例如，下图显示了如何使用 `stepNum` 参数定义 [Quick Start](./快速开始.md) 任务。

   <img src="./imgs/use-parameters.png" width="100%" height="100%" />

你可以定义batch size、学习率或任何你希望的地方为参数，以加快提交作业的速度。

在某些情况下，需要定义一些私密信息，例如密码、令牌等。你可以利用 `Secrets` 部分来提供信息。用法与 Parameters 部分相同，只是这些私密信息不会被显示和记录。

## <div id="multiple-task-roles">多任务角色</div>

如果你使用 `Distributed` 按钮来提交任务，则可以为任务添加不同的 task role。

   <img src="./imgs/distributed-job.png" width="60%" height="60%" />

什么是 task role？对于单服务器任务，只有一个 task role。对于分布式任务，可能有多个 task role。例如，当TensorFlow用于运行分布式任务时，它具有两个角色，包括参数服务器（parameter server）和工人（worker）。

下图中的 `Instances` 是该 task role 的 instance 数量。在分布式任务中，它依赖于一个 task role 需要多少个 instance。例如，如果TensorFlow的一个工人角色设置 Instances 为 8，就意味着这个工人角色应该有8个Docker容器。
...
一些环境变量与端口有关。在OpenPAI中，你可以在高级设置中为每个task保留端口，如下图所示：
...
*注意你需要使用 `$PAI_CURRENT_TASK_ROLE_NAME` 和 `$PAI_CURRENT_TASK_ROLE_CURRENT_TASK_INDEX`*

- 使用 **即将弃用的** 环境变量来获取当前容器中的端口（列表），例如
```bash
...
在任务中，瞬时错误将一直重试，永久错误永远不会重试。如果发生未知错误，PAI 将根据用户设置来重试任务。要为任务设置重试策略和完成策略，请切换至 `Advanced` 模式，如下图所示：
...
在 `Completion policy` 中，设置了 `Min Failed Instances` 和 `Min Succeed Instances`。`Min Failed Instances` 是指会使整个任务失败的失败 task 数量。它应该为 -1 或不小于 1。如果将它设置为 -1，不论多少 task 失败，任务都将会成功。默认值为 1，这意味着 1 个失败的 task 将会导致整个任务的失败。 `Min Succeed Instances` 表示会使整个任务成功的成功 task 数量。它应该是 -1 或不小于 1。如果将其设置为 -1，则任务只会在所有 task 完成且未触发 `Min Failed Instances` 时成功。默认值是 -1。

如果任务满足 `Completion policy` 之后仍没有成功，失败是由于未知错误引起的，并且
 `Retry count` 是大于0的，那么将会重试整个任务。如果你需要更多次重试，将 `Retry count` 设置为更大的数字即可。

最后，对于 `Task retry count`，它是单个 task 的最大重试次数。需要特别注意的是，除非你在 [任务协议](#job-protocol-export-and-import-jobs) 里将 `extras.gangAllocation` 设置为 `false`，否则此设置不会生效。
...
你还可以使用 `Export` 和 `Import` 按钮导出和导入YAML文件：
...
此示例是 Tensorflow CIFAR-10 训练任务，它运行了一个参数服务器（parameter server）和一个工人（worker）。这项任务需要至少 5 个 GPU。请参考 [tensorflow-cifar10.yaml](https://github.com/microsoft/pai/blob/master/marketplace-v2/tensorflow-cifar10.yaml)。

### Horovod PyTorch

此示例，[horovod-pytorch-synthetic-benchmark.yaml](https://github.com/microsoft/pai/blob/master/marketplace-v2/horovod-pytorch-synthetic-benchmark.yaml)，是一个使用了PyTorch 和 Open MPI 的 Horovod benchmark。请确保任务 yaml 中的 `IFNAME` 设置适合你的环境。它至少需要 8 个 GPU。
