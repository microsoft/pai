...
OpenPAI的架构在`v1.0.0`中进行了更新和优化。在`v1.0.0`之前，OpenPAI基于Yarn和Kubernetes，数据由HDFS管理。从`v1.0.0`开始，OpenPAI转变为纯Kubernetes的架构。除此之外，`v1.0.0`还包括许多新特性，如`AAD 认证`、`Hived调度器`、`Kube Runtime`、`Marketplace`等。如果您仍要安装旧的基于Yarn的OpenPAI，请使用`v0.14.0`。
...
目前，OpenPAI还不支持高可用（HA），它只能使用一个master机器。我们将来将添加HA功能。另外，您不能只使用一台机器来部署OpenPAI。您必须有一个dev box机器，一个master机器和至少一个worker机器。如果您想要在一台机器部署，请在[Github](https://github.com/microsoft/pai)提交一个功能请求.

#### 使用纯CPU worker的提示

目前，在安装脚本中，我们还不支持纯CPU的worker机器。如果您同时拥有GPU worker和CPU worker，请先使用GPU workers安装PAI。成功安装之后，您可以将CPU worker附加进来，并设置一个纯CPU的虚拟集群，相关步骤请参阅[如何添加和移除结点](./如何添加和移除结点.md)。 如果你只有CPU worker，我们还没有正式的安装支持。请在[Github](https://github.com/microsoft/pai)提交功能请求issue。
...
正如这个提示，您可以用 `admin`  `admin-password` 来登录Webportal，并提交一个任务来验证安装。
